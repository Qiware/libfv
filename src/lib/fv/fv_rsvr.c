...
#define FV_RSVR_DIST_POP_NUM   (1024)

static fv_rsvr_t *fv_rsvr_self(fv_cntx_t *ctx);
static int fv_rsvr_add_conn(fv_cntx_t *ctx, fv_rsvr_t *rsvr);
static int fv_rsvr_del_conn(fv_cntx_t *ctx, fv_rsvr_t *rsvr, socket_t *sck);

static int fv_recv_data(fv_cntx_t *ctx, fv_rsvr_t *rsvr, socket_t *sck);
static int fv_send_data(fv_cntx_t *ctx, fv_rsvr_t *rsvr, socket_t *sck);

static int fv_rsvr_dist_send_data(fv_cntx_t *ctx, fv_rsvr_t *rsvr);
static socket_t *fv_push_into_send_list(fv_cntx_t *ctx, fv_rsvr_t *rsvr, uint64_t cid, void *addr);

static int fv_rsvr_kick_conn(fv_cntx_t *ctx, fv_rsvr_t *rsvr);

static int fv_rsvr_event_hdl(fv_cntx_t *ctx, fv_rsvr_t *rsvr);
static int fv_rsvr_timeout_hdl(fv_cntx_t *ctx, fv_rsvr_t *rsvr);

static int fv_rsvr_connection_cmp(const int *cid, const socket_t *sck);

/******************************************************************************
 **函数名称: fv_rsvr_routine
 **功    能: 运行接收线程
 **输入参数:
 **     _ctx: 全局对象
 **输出参数: NONE
...
{
    fv_rsvr_t *rsvr;
    fv_cntx_t *ctx = (fv_cntx_t *)_ctx;

    nice(-20);

    /* > 获取代理对象 */
    rsvr = fv_rsvr_self(ctx);
    if (NULL == rsvr) {
    ...
    while (1) {
        ...
        rsvr->fds = epoll_wait(rsvr->epid, rsvr->events,
                FV_EVENT_MAX_NUM, FV_TMOUT_MSEC);
        if (rsvr->fds < 0) {
            if (EINTR == errno) {
                continue;
            }
            ...
            rsvr->ctm = time(NULL);
            if (rsvr->ctm - rsvr->scan_tm > FV_TMOUT_SCAN_SEC) {
                rsvr->scan_tm = rsvr->ctm;
                fv_rsvr_timeout_hdl(ctx, rsvr);
            }
            continue;
        }

        /* > 进行事件处理 */
        fv_rsvr_event_hdl(ctx, rsvr);
    }
...
/* 命令接收处理 */
static int agt_rsvr_recv_cmd_hdl(fv_cntx_t *ctx, fv_rsvr_t *rsvr, socket_t *sck)
{
    cmd_data_t cmd;
    ...
    while (1) {
        ...
        switch (cmd.type) {
            case CMD_ADD_SCK:
                if (fv_rsvr_add_conn(ctx, rsvr)) {
                    log_error(rsvr->log, "Add connection failed！");
                }
                break;
            case CMD_DIST_DATA:
                if (fv_rsvr_dist_send_data(ctx, rsvr)) {
                    log_error(rsvr->log, "Disturibute data failed！");
                }
                break;
            case CMD_KICK_CONN:
                if (fv_rsvr_kick_conn(ctx, rsvr)) {
 ...
 ******************************************************************************/
int fv_rsvr_init(fv_cntx_t *ctx, fv_rsvr_t *rsvr, int idx)
{
    struct epoll_event ev;
    fv_socket_extra_t *extra;
    ...
    do {
        /* > 创建epoll对象 */
        rsvr->epid = epoll_create(FV_EVENT_MAX_NUM);
        if (rsvr->epid < 0) {
            log_error(rsvr->log, "errmsg:[%d] %s!", errno, strerror(errno));
            break;
        }

        rsvr->events = calloc(1, FV_EVENT_MAX_NUM * sizeof(struct epoll_event));
        if (NULL == rsvr->events) {
            log_error(rsvr->log, "errmsg:[%d] %s!", errno, strerror(errno));
            break;
        }
        ...
        /* > 创建附加信息 */
        extra = calloc(1, sizeof(fv_socket_extra_t));
        if (NULL == extra) {
    ...
    fv_rsvr_destroy(rsvr);
    return FV_ERR;
}

/******************************************************************************
 **函数名称: fv_rsvr_sck_dealloc
 **功    能: 释放SCK对象的空间
 **输入参数:
 **     pool: 内存池
 **     sck: 需要被释放的套接字对象
 ...
 ******************************************************************************/
static int fv_rsvr_sck_dealloc(void *pool, socket_t *sck)
{
    fv_socket_extra_t *extra = sck->extra;
 ...
 **作    者: # Qifeng.zou # 2016.11.18 #
 ******************************************************************************/
int fv_rsvr_destroy(fv_rsvr_t *rsvr)
{
    FREE(rsvr->events);
    CLOSE(rsvr->epid);
...
/******************************************************************************
 **函数名称: fv_rsvr_get_timeout_conn_list
 **功    能: 将超时连接加入链表
 **输入参数: 
 **     node: 平衡二叉树结点
 **     timeout: 超时链表
 ...
 ******************************************************************************/
static int fv_rsvr_get_timeout_conn_list(
        fv_socket_extra_t *extra, fv_conn_timeout_list_t *timeout)
{
#define FV_SCK_TIMEOUT_SEC (180)
    socket_t *sck = extra->sck;

    /* 判断是否超时，则加入到timeout链表中 */
    if ((timeout->ctm - sck->rdtm <= FV_SCK_TIMEOUT_SEC)
        || (timeout->ctm - sck->wrtm <= FV_SCK_TIMEOUT_SEC))
    {
        return FV_OK; /* 未超时 */
    }
 ...
 ******************************************************************************/
static int fv_rsvr_conn_timeout(fv_cntx_t *ctx, fv_rsvr_t *rsvr)
{
    socket_t *sck;
    fv_socket_extra_t key;
    fv_conn_timeout_list_t timeout;
    ...
    do {
        ...
        hash_tab_trav_slot(ctx->conn_cid_tab, &key,
                (trav_cb_t)fv_rsvr_get_timeout_conn_list, &timeout, RDLOCK);
...
static int fv_rsvr_timeout_hdl(fv_cntx_t *ctx, fv_rsvr_t *rsvr)
{
    fv_rsvr_conn_timeout(ctx, rsvr);
    return FV_OK;
}

/******************************************************************************
 **函数名称: fv_rsvr_add_conn
 **功    能: 添加新的连接
 **输入参数: 
 **     ctx: 全局信息
 **     rsvr: 接收服务
...
static int fv_rsvr_add_conn(fv_cntx_t *ctx, fv_rsvr_t *rsvr)
{
#define FV_RSVR_CONN_POP_NUM (1024)
    int num, idx;
    socket_t *sck;
    queue_t *connq;
    struct epoll_event ev;
    ...
    fv_socket_extra_t *extra;
    fv_add_sck_t *add[FV_RSVR_CONN_POP_NUM];

    connq = rsvr->connq;
    while (1) {
        num = MIN(queue_used(connq), FV_RSVR_CONN_POP_NUM);
        if (0 == num) {
            return FV_OK;
        }
        ...
        for (idx=0; idx<num; ++idx) {
            ...
            /* > 创建SCK关联对象 */
            extra = (fv_socket_extra_t *)calloc(1, sizeof(fv_socket_extra_t));
            if (NULL == extra) {
                log_error(rsvr->log, "Alloc memory failed! cid:%lu", add[idx]->cid);
                CLOSE(add[idx]->fd);
                FREE(sck);
            ...
            sck->recv_cb = (socket_recv_cb_t)fv_recv_data;   /* Recv回调函数 */
            sck->send_cb = (socket_send_cb_t)fv_send_data;   /* Send回调函数*/

            queue_dealloc(connq, add[idx]);      /* 释放连接队列空间 */

            /* > 插入红黑树中(以序列号为主键) */
            if (fv_conn_cid_tab_add(ctx, extra)) {
            ...
            ctx->protocol->callback(ctx, sck, FV_CALLBACK_SCK_CREAT,
...
static int fv_rsvr_del_conn(fv_cntx_t *ctx, fv_rsvr_t *rsvr, socket_t *sck)
{
    ...
    /* > 剔除CID对象 */
    fv_conn_cid_tab_del(ctx, extra->cid);

    /* > 释放套接字空间 */
    CLOSE(sck->fd);

    ctx->protocol->callback(ctx, sck, FV_CALLBACK_SCK_CLOSED,
            (void *)extra->user, (void *)NULL, 0, (void *)ctx->protocol->args);
    ctx->protocol->callback(ctx, sck, FV_CALLBACK_SCK_DESTROY,
            (void *)extra->user, (void *)NULL, 0, (void *)ctx->protocol->args);
 ...
 ******************************************************************************/
static int fv_recv_head(fv_cntx_t *ctx,
        fv_rsvr_t *rsvr, socket_t *sck, size_t per_packet_head_size)
{
    void *addr;
    int n, left;
 ...
 ******************************************************************************/
static int fv_recv_body(fv_rsvr_t *rsvr, socket_t *sck)
{
    int n, left;
    socket_snap_t *recv = &sck->recv;
 ...
 **作    者: # Qifeng.zou # 2016.09.17 #
 ******************************************************************************/
static int fv_recv_post_hdl(fv_cntx_t *ctx, fv_rsvr_t *rsvr, socket_t *sck)
{
    fv_protocol_t *protocol = ctx->protocol;
    fv_socket_extra_t *extra = (fv_socket_extra_t *)sck->extra;

    return protocol->callback(ctx, sck, FV_CALLBACK_RECEIVE,
...
static int fv_recv_data(fv_cntx_t *ctx, fv_rsvr_t *rsvr, socket_t *sck)
{
    ...
    for (;;) {
        switch (recv->phase) {
            ...
            RECV_HEAD:
                ret = fv_recv_head(ctx, rsvr, sck, protocol->per_packet_head_size);
                switch (ret) {
                    case FV_OK:
            ...
            RECV_POST:
                /* 对接收到的数据进行处理 */
                ret = fv_recv_post_hdl(ctx, rsvr, sck);
                mref_dec(recv->addr);
                recv->addr = NULL;
                if (FV_OK == ret) {
...
static int fv_rsvr_dist_send_data(fv_cntx_t *ctx, fv_rsvr_t *rsvr)
{
    ...
    fv_send_item_t *item;
    struct epoll_event ev;
    void *addr[FV_RSVR_DIST_POP_NUM];

    sendq = rsvr->sendq;
    while (1) {
        num = MIN(ring_used(sendq), FV_RSVR_DIST_POP_NUM);
        if (0 == num) {
            break;
        }
        ...
        for (idx=0; idx<num; ++idx) {
            item = (fv_send_item_t *)addr[idx];

            /* > 发入发送列表 */
            sck = fv_push_into_send_list(ctx, rsvr, item->cid, addr[idx]); 
            if (NULL == sck) {
 ...
 ******************************************************************************/
static socket_t *fv_push_into_send_list(
        fv_cntx_t *ctx, fv_rsvr_t *rsvr, uint64_t cid, void *addr)
{
    socket_t *sck;
...
static int fv_rsvr_kick_conn(fv_cntx_t *ctx, fv_rsvr_t *rsvr)
{
    ...
    kickq = rsvr->kickq;
    while (1) {
        num = MIN(queue_used(kickq), FV_RSVR_DIST_POP_NUM);
        if (0 == num) {
            break;
        }
