...
static int fv_lsvr_timeout_hdl(fv_cntx_t *ctx, fv_lsvr_t *lsvr);
static int fv_lsvr_accept(fv_cntx_t *ctx, fv_lsvr_t *lsvr);
static int fv_lsvr_send_add_sck_req(fv_cntx_t *ctx, fv_lsvr_t *lsvr, int idx);
 ...
 ******************************************************************************/
static fv_lsvr_t *fv_lsn_self(fv_cntx_t *ctx)
{
    int id;
    fv_lsvr_t *lsvr;
...
void *fv_lsvr_routine(void *_ctx)
{
    ...
    while (1) {
        ...
        if (ret < 0) {
            ...
            fv_lsvr_timeout_hdl(ctx, lsvr);
            continue;
        }

        /* > 接收网络连接 */
        if (FD_ISSET(ctx->listen.lsn_sck_id, &rdset)) {
            fv_lsvr_accept(ctx, lsvr);
        }
    }
    return (void *)0;
}
...
static int fv_lsvr_timeout_hdl(fv_cntx_t *ctx, fv_lsvr_t *lsvr)
{
    ...
    for (idx=0; idx<ctx->conf->rsvr_num; ++idx) {
        if (queue_used(ctx->connq[idx])) {
            fv_lsvr_send_add_sck_req(ctx, lsvr, idx);
        }
    }
...
static int fv_lsvr_accept(fv_cntx_t *ctx, fv_lsvr_t *lsvr)
{
    ...
    add = queue_malloc(ctx->connq[idx], sizeof(fv_add_sck_t));
    if (NULL == add) {
        log_error(lsvr->log, "Alloc from queue failed! fd:%d size:%d/%d",
                fd, sizeof(fv_add_sck_t), queue_size(ctx->connq[idx]));
        CLOSE(fd);
        return FV_ERR;
    }
