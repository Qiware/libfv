 ...
 ******************************************************************************/
static int fv_send_cmd_to_rsvr(fv_cntx_t *ctx, int rid, int cmd_id)
{
    cmd_data_t cmd;
...
int fv_async_send(fv_cntx_t *ctx, int type, uint64_t cid, void *data, int len)
{
    ...
    item = (void *)calloc(1, sizeof(fv_send_item_t));
    if (NULL == item) {
    ...
    fv_send_cmd_to_rsvr(ctx, rid, CMD_DIST_DATA); /* 发送分发命令 */
...
int fv_async_kick(fv_cntx_t *ctx, uint64_t cid)
{
    ...
    kick = queue_malloc(kickq, sizeof(fv_kick_req_t));
    if (NULL == kick) {
    ...
    fv_send_cmd_to_rsvr(ctx, rid, CMD_KICK_CONN); /* 发送分发命令 */
